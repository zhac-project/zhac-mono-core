...
- One `"` or `\` (or a control character) in a device's name, model or manufacturer made the whole `device.list` reply invalid JSON: the rows pasted the strings in unescaped, so the web UI's device table failed to load (and the cloud, with the link on, stored no device). Row strings are now JSON-escaped (UTF-8 passes through unchanged); a name made of nothing but quotes or control characters is cut at a character boundary rather than overflowing the row. Same fix as zhac-wired-core. Not yet run on hardware.
